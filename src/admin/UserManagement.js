  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Fetch patients (keep this the same)
        ...
        const patientsData = patientsSnapshot.docs.map((doc) => ({
          id: doc.data().User_ID,
          firstName: doc.data().User_FName,
          lastName: doc.data().User_LName,
          middleInitial: doc.data().User_MName?.charAt(0) || "",
          email: doc.data().User_Email,
          plan: "Basic",
          status: "Active",
          phoneNumber: doc.data().User_PhoneNumber,
          address: doc.data().User_HomeAddress,
        ...
        const dermData = await Promise.all(
          dermSnapshot.docs.map(async (doc) => {
            ...
            let clinicInfo = {};
            try {
              const clinicDermQuery = query(
                collection(db, "ClinicDermatologist"),
                where("Dermatologist_ID", "==", userData.User_ID)
              );
              const clinicDermSnapshot = await getDocs(clinicDermQuery);
              ...
              if (!clinicDermSnapshot.empty) {
                ...
                if (clinicDoc.exists()) {
                  ...
                  clinicInfo = {
                    ...
                    clinicSchedule: `${clinic.Clinic_StartTime} - ${clinic.Clinic_EndTime}`,
                  };
                }
              }
            } catch (error) {
            ...
            return {
              ...
              middleInitial: userData.User_MName?.charAt(0) || "",
              email: userData.User_Email,
              specialization: "General Dermatology",
              license: userData.User_ValidDermatologist_ID ? "Yes" : "No",
              status: userData.User_ValidDermatologist_ID
                ? "Active"
                : "Pending",
        ...
        setRealPatients(patientsData);
        setRealDermatologists(dermData);
      } catch (error) {
 ...
 const handleApprove = async (userId) => {
   try {
    const user = realDermatologists.find((d) => d.id === userId);
     ...
     setRealDermatologists((prev) =>
       prev.map((derm) =>
         derm.id === userId
           ? { ...derm, license: "Yes", status: "Active", isApproved: true }
           : derm
       )
     );
     await logActivity(
     ...
     alert("Failed to approve dermatologist. Please try again.");
   }
 };
  const handleReject = async (userId) => {
    try {
      ...
      setRealDermatologists((prev) =>
        prev.map((derm) =>
          derm.id === userId
            ? { ...derm, license: "No", status: "Rejected", isApproved: false }
            : derm
        )
      );
       await logActivity(
      ...
      alert("Failed to reject dermatologist. Please try again.");
    }
  };
  const handleDeactivateUser = async (userId) => {
    if (
      ...
      try {
        // You'll need to add this function to your AuthContext
        ...
        if (activeTab === "patient") {
          ...
          setRealDermatologists((prev) =>
            prev.map((derm) =>
              derm.id === userId ? { ...derm, status: "Inactive" } : derm
            )
          );
        }
  ...
  const columns = activeTab === 'patient'
    ...
    : [
      ...
      if (loading) {
    return (
      <div className="user-management-main d-flex justify-content-center align-items-center" style={{ height: '50vh' }}>
  ...
  const dermKPI = [
    {
      label: "Total Dermatologists",
      value: realDermatologists.length,
      change: "+0%", // You can calculate this if you track historical data
      up: true,
    },
    {
      label: "Approved",
      value: realDermatologists.filter((d) => d.isApproved).length,
      change: "+0%",
      up: true,
    },
    {
    ...
    <Container fluid className="responsive-user-mgmt-container">
      ...
      <Row className="align-items-start mb-3 flex-wrap user-mgmt-header-row">
        <Col
          ...
          md={4}
          lg={4}
          className="mb-2 mb-lg-0 d-flex align-items-start justify-content-lg-start justify-content-start"
        >
        ...
        <Col
          ...
          md={5}
          lg="auto"
          className="d-flex align-items-start justify-content-lg-start justify-content-start mb-2 mb-lg-0"
        >
      ...
      <div className="d-flex flex-wrap align-items-center mb-3 gap-4 user-mgmt-filter-row">
        ...
        <InputGroup
          size="sm"
          className="user-mgmt-search-group"
          style={{ maxWidth: 250 }}
        >
          <InputGroup.Text
          ...
          >
            <img
              ...
              alt="Search"
              className="user-mgmt-search-icon"
            />
          </InputGroup.Text>
          {!isMobile && (
            <FormControl
                ...
                <Table hover size="lg" className="user-table align-middle">
                  ...
                  <tbody>
                    {paginated.map((row, idx) => (
                      <tr
                        key={row.id}
                        className={idx % 2 === 0 ? "table-row-alt" : ""}
                      >
                        {columns.map((col) => {
                          if (col.key === "action") {
                            return (
                          ...
                          if (col.key === "status") {
                            return (
                              <td key={col.key}>{statusBadge(row.status)}</td>
                            );
                          }
          ...
          <div className="kpi-panel d-flex flex-column align-items-stretch user-kpi-panel-col">
            {activeTab === "patient" ? (
              <>
                ...
                <Card className="kpi-card subscription-card mb-3 mobile-kpi-spacing">
                  <Card.Body className="p-3 position-relative">
                    <div className="d-flex align-items-center justify-content-between mb-1">
                      ...
                      <img
                        src="/icons/today_subscriber.png"
                        alt="Wallet"
                        className="user-mgmt-revenue-icon"
                      />
                    </div>
                    ...
                    <div className="kpi-change text-danger mt-2 d-flex align-items-center user-mgmt-kpi-change">
                      -20%{" "}
                      <span className="ms-1 user-mgmt-kpi-subtext">
                        than last month
                      </span>
                    </div>
                  </Card.Body>
                ...
                <Card className="kpi-card subscription-card mb-3 mobile-kpi-spacing">
                  <Card.Body>
                    ...
                    <div className="subscription-value fw-bold text-dark">
                      123{" "}
                      <span className="kpi-subscribtions">Subscriptions</span>
                    </div>
                ...
                <Card className="kpi-card subscription-card mb-3 mobile-kpi-spacing">
                  <Card.Body>
                    ...
                    <div className="subscription-growth d-flex align-items-center mt-2">
                      <span className="me-1 label">&#128201;</span>
                      <span className="text-danger success">
                        -10% than last month
                      </span>
                    </div>
                  </Card.Body>
              ...
              <div className="d-flex flex-column justify-content-start align-items-start w-100">
                {dermKPI.map((k, i) => (
                  <Card
                    className=" subscription-card mb-3 mobile-kpi-spacing"
                    key={k.label}
                  >
                    <Card.Body>
                      <div className="subscription-header text-primary fw-bold">
        ...
        {activeTab === "derm" ? (
          <div className="user-details-modal-card user-details-modal-card-with-header">
            ...
            <div className="user-details-modal-body">
              ...
              <div className="user-details-modal-overview">
                <div>
                  <b>User ID:</b> <span>D-{selectedUser?.id}</span>
                </div>
                <div>
                  <b>Status:</b>{" "}
                  <span>
          ...
          <div className="user-details-modal-card user-details-modal-card-with-header">
            ...
            <div className="user-details-modal-body">
              ...
              <div className="user-details-modal-overview">
                <div>
                  <b>User ID:</b> <span>U-{selectedUser?.id}</span>
                </div>
                <div>
                  <b>Status:</b> <span>{selectedUser?.status}</span>
                </div>
                <div>
                  <b>Plan Type:</b> <span>{selectedUser?.plan}</span>
                </div>
              </div>
              {/* Close Button */}
              <button
